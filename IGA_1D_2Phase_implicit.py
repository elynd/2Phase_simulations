...
from scipy.optimize import newton_krylov, fsolve, anderson,newton,root, fsolve,broyden1,excitingmixing
...
#        print(self.R_list)
    ...
    def relative_permeability(self,Sw,deriv=0):
        ...
        if deriv==0:
            ...
            return (self.k_wr*self.lam_1)*Sw_norm**(self.lam_1-1)*1/(S_m-Sw_irr),(self.k_nwr*self.lam_2)*(1-Sw_norm)**(self.lam_2-1)*(-1)/(S_m-Sw_irr)
        elif deriv==2:
            return (self.k_wr*self.lam_1*(self.lam_1-1)*Sw_norm**(self.lam_1-2)*(1/(S_m-Sw_irr))**2),(self.k_nwr*self.lam_2*(self.lam_2-1)*Sw_norm**(self.lam_2-2)*(1/(S_m-Sw_irr))**2)
    ...
    def F_w(self,Sw,deriv=0):
        ...
        """
        krw,kro=self.relative_permeability(Sw);  dkrw_dSw,dkro_dSw=self.relative_permeability(Sw,1);dkrw_dSw2,dkro_dSw2=self.relative_permeability(Sw,2);
#        q=self.source_values_flux
#        print(kro)
        mu_o=self.mu_o;mu_w=self.mu_w
        ...
        if deriv==0:
            ...
            dfw_dSw2=(mu_o*mu_w*(-krw**2*mu_o*dkro_dSw2+kro*(-2*mu_o*dkrw_dSw**2+mu_w*(-2*dkro_dSw*dkrw_dSw+kro*dkrw_dSw2)) + \
            krw*(mu_w*(2*dkro_dSw**2 - kro*dkro_dSw2) + mu_o*(2*dkro_dSw*dkrw_dSw+kro*dkrw_dSw2))))/(krw*mu_o+kro*mu_w)**3
            return np.nan_to_num(dfw_dSw2)
...
#        saturation[]
        ...
        if self.dimension==1:
            ...
            R_s=np.zeros(len(self.B))
            
            R_s_VMS=np.zeros(len(self.B))
            R_SCF=np.zeros(len(self.B))

            Q_s=np.zeros(len(self.B))
...
#            Q=np.zeros(len(self.B)*2)
            detJ_in,R_in,dR_dx_in=self.mappings(0,0,np.array([-1]))
            detJ_out,R_out,dR_dx_out=self.mappings(0,self.number_elements-1,np.array([1]))
            ...
            Q_s[0]+=self.source_values_flux/self.phi-Dc_in*dSw_dx_in          
            Q_s[-1]+=q_w_out/self.phi+Dc_out*dSw_dx_out
           ...
           #Looping through patches
            for aa in range(number_patches):
                ...
                for bb in range(nelem): 
                    ...
                    a=np.einsum('ij,j->ij',R,detJ*self.wl)
                    d=np.einsum('ij,j->ij',dR_dx,detJ*self.wl*Dc)
                    e=np.einsum('ij,j->i',dR_dx,detJ*self.wl*self.source_values_flux[0]/self.phi*fw)

                    ###Forming entries of overall partitioned matrices
                    A=np.einsum('ji,ki->jk',R,a)      
                    D=np.einsum('ji,ki->jk',dR_dx,d)       
                    K1=A/self.delta_t+D; K2=A/self.delta_t;

                    R_s[element_indeces]+= np.dot(K1,Sw)-np.dot(K2,self.previous_saturation[element_indeces])-e-Q_s[element_indeces]
                    ...
                    if self.SG==1:
                        ...
                        L_adj=(-L_1-L_2)
                        L_adj=np.nan_to_num(L_adj)
                        
                        a_VMS=np.einsum('ij,j->ij',R,detJ*self.wl)
                        d1_VMS=np.einsum('ij,j->ij',dR_dx,detJ*self.wl*dDc_dSw*dSw_dx)
                        d2_VMS=np.einsum('ij,j->ij',dR_dx2,detJ*self.wl*Dc)
                        e_VMS=np.einsum('ij,j->ij',dR_dx,detJ*self.wl*self.source_values_flux[0]/self.phi*dfw_dSw)
                        
                        K1=a_VMS/self.delta_t-(d1_VMS+d2_VMS)+e_VMS;
                        K2=a_VMS/self.delta_t;

#                        Resid= -np.einsum('ji,j->ji',K1,Sw)+np.einsum('ji,j->ji',K2,self.previous_saturation[element_indeces])
#                        print(Resid)

                        Resid_tau_n=np.einsum('ji,ki->jk',L_adj,np.einsum('ij,j->ij',K1,tau)); Resid_tau_nmin1=np.einsum('ji,ki->jk',L_adj,np.einsum('ij,j->ij',K2,tau))
                        R_s_VMS[element_indeces]+=  -np.dot(Resid_tau_n,Sw)+np.dot(Resid_tau_nmin1,self.previous_saturation[element_indeces])
...
#
#            L_1_in=dR_dx_in[0,0]*a1_in; L_1_out=dR_dx_out[-1,-1]*a1_out
#            L_2_in=dR_dx2_in[0,0]*Dc_in;  L_2_out=dR_dx2_out[-1,-1]*Dc_out
...
#            R_s_VMS+= Q_VMS
            ...
            R_s_VMS[self.boundary_saturations_locations]=0;
            R_SCF[self.boundary_saturations_locations]=0;

            R_s_VMS=np.nan_to_num(R_s_VMS)
            R_s[self.boundary_saturations_locations]=0; 

            return R_s+R_s_VMS+R_SCF
    ...
    def create_solution(self):
        ...
        for t in range(1,len(self.times)):
            ...
            self.previous_saturation=self.saturations[:,t-1];  
            init_guess=np.copy(self.saturations[:,t-1])
            init_guess[self.boundary_saturations_locations]=self.boundary_saturations_values
            self.saturations[:,t]=newton_krylov(self.assemble,init_guess,verbose=1,maxiter=100,f_tol=10**-3)
            toc=time.time()-tic
            print(t)
...
#        writer='mencoder'
    ...
    def make_frame_s(self,num_pts=200,BL=False):
...
#            if j!=0:
#                x_BL,Sw_BL=Buckley_Leverett(np.array([self.mu_w,self.mu_o]),np.array([self.phi,self.S_wirr,self.S_m]),self.rel_perm_params,self.source_values_flux[0],1,self.k[0],self.B[-1,0],self.times[j]).sat_curve(num_pts)
##                print(Sw_BL)
#                ax,plot(x_BL,Sw_BL)
            plt.xlabel('x')
