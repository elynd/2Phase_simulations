...
#        print(tau)
    ...
    def assemble(self,saturation,pressure,delta_t,capillary_pressure=False,test=False,mode='implicit'):
        ...
        if self.dimension==1:
            ...
            E=np.zeros((number_global_bases,number_global_bases))
...
#            -eps*self.k/self.phi*dSw_dx_in
            Q[len(self.B)-1]+=(q_o_out+q_w_out)/self.phi
#            +eps*self.k/self.phi*dSw_dx_out
...
#                    print(R)
                    ...
                    fw=self.F_w(Sw_pts)
                    dfw_dSw=self.F_w(Sw_pts,1)
                    dfw_dSw_in=self.F_w(Sw_in,1);  dfw_dSw_out=self.F_w(Sw_out,1)
                    ###Diffusion coeff
                    Sw_norm=(Sw_pts-self.S_wirr)/(self.S_m-self.S_wirr)
                    Sw_norm_in=(Sw_in-self.S_wirr)/(self.S_m-self.S_wirr) ;Sw_norm_out=(Sw_out-self.S_wirr)/(self.S_m-self.S_wirr)
                    eps=.00001
                    Dc=eps*(Sw_norm-Sw_norm**2)
                    Dc_in=eps*(Sw_in-Sw_in**2);Dc_out=eps*(Sw_in-Sw_in**2)


                    dDc_dSw=eps*(1./(self.S_m-self.S_wirr))*(1-2*Sw_norm)
                    dDc_dSw_in=eps*(1/(self.S_m-self.S_wirr))*(1-2*Sw_norm_in);dDc_dSw_out=eps*(1/(self.S_m-self.S_wirr))*(1-2*Sw_norm_out)
...
#                    e=np.einsum('ij,j->ij',dR_dx,detJ*wl*self.source_values_flux[0]/self.phi*dfw_dSw)
...
#                    E[idx_grid]+=np.einsum('ji,ki->jk',R,e)  
#                    E[idx_grid]+=np.einsum('ji,ki->jk',dR_dx,e)  
                    Q[len(self.B)+element_indeces]+=e
...
#                        np.einsum('ij,j->ij',dR_dx,a2)+np.einsum('ij,j->ij',dR_dx2,Dc)
                        L=-L_1-L_2
#                        L=-L_1
#                        print(L)
                        ...
                        d1_VMS=np.einsum('ij,j->ij',dR_dx,tau*detJ*wl*dDc_dSw)
                        d2_VMS=np.einsum('ij,j->ij',dR_dx2,tau*detJ*wl*Dc)

                    
#                        e_VMS_tau=np.einsum('ij,j->ij',dR_dx*L,tau*detJ*wl*self.source_values_flux[0]/self.phi*dfw_dSw)
                        ...
                        D1_VMS[idx_grid]+=np.einsum('ji,ki->jk',L,d1_VMS)
                        D2_VMS[idx_grid]+=np.einsum('ji,ki->jk',L,d2_VMS)

                        E_VMS[idx_grid]+=np.einsum('ji,ki->jk',L,e_VMS)
#                        print(A_VMS)
...
#                        K1_VMS=-(C_VMS+B_VMS)
#                        K2_VMS=-B_VMS_tau   
#                        K3_VMS=A_VMS/(delta_t)-D_VMS_tau+E_VMS_tau
                        K3_VMS=A_VMS/(delta_t)-(D1_VMS+D2_VMS)+E_VMS
...
#                        print(tau)
                ...
                Q_VMS[len(self.B)]+=L_in*tau_in*Q[len(self.B)]
...
#            print(K2)
    ...
    def residuals(self,nodal_vals_vec=None,mode='implicit'):
        ...
        if mode=='explicit':
            K1,K2,Q=self.boundary_conditions(saturation_init,pressure_initl,delta_t,mode=mode)
#            print K1
#            print K2
#            print Q
        ...
        if mode=='implicit':
            ...
            K1,K2,Q,Q_VMS=self.boundary_conditions(saturation,pressure,delta_t,mode=mode)
            R=np.einsum('ij,j->i',K1,np.hstack((pressure,saturation)))-np.einsum('ij,j->i',K2,np.hstack((pressure_init,saturation_init)))-Q-Q_VMS   
...
#                print(self.pressure_init)
                ...
                Q=Q[len(self.B):]
                Q_VMS=Q_VMS[len(self.B):]
                s_current=np.linalg.solve(K1_s,np.einsum('ij,j->i',K2_s,self.saturation_init)+Q+Q_VMS)
